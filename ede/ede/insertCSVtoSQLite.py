...
class insert:
    def __init__(self, args):
        ...
        self.args.path_to_DB_file = self.cargarBaseDeDatos(
            f'{self.args.t_stamp}_encryptedD3.db')
    ...
    def cargarBaseDeDatos(self, fileName: str) -> str:
        ...
        _t = f"Base de datos: '{path_to_DB_file}' creada exitosamente "
        logger.info(_t)
    ...
    def getPublicKeyFromEmail(self, email: str) -> Union[str, None]:
        ...
        try:
            ...
            _t = f"clave pública: SIN INFORMACIÓN"
            logger.error(_t)
            return None
    ...
    def encriptarBD(self, DB_NAME: str) -> str:
        ...
        engine = create_engine(
            f"sqlite+pysqlcipher3://:{secPhase}@/{DB_NAME}?cipher=aes-256-cfb&kdf_iter=64000")
        conn = engine.connect()
        ...
        conn.close()
        _t = 'Cambio de clave a la BD realizado exitosamente'
        logger.info(_t)
        return psw
    ...
    def encriptarClaveParaLaSuperintendencia(self) -> Literal[True]:
        text_file = open(f"{self.args.t_stamp}_key.txt", "w")
        text_file.write(self.args.secPhase)
        text_file.close()
        secPhaseEncripted = self.encryptTextUsingSiePublicKey(
            self.args.secPhase)
        text_file = open(f"{self.args.t_stamp}_key.encrypted", "wb")
        text_file.write(secPhaseEncripted)
        text_file.close()
        _t = 'Archivo encriptado de la SIE generado exitosamente'
        logger.info(_t)
        return True
    ...
    def dict_factory(self, rows, tbl) -> Dict:
        ...
        for col in rows:
            ...
            try:
                ...
                logger.info(
                    f"ERROR: Tipo de datos {tbl}->{col[2]} no encontrado, se procesará como 'np.unicode_' ...")
                d[col[1]] = np.unicode_
        return d
    ...
    def transferCSVToSQL_withPandas(self, path_to_dir_csv_file, DB_NAME: str, secPhase: str) -> bool:
        ...
        for root, dirs, files in os.walk(path_to_dir_csv_file, topdown=False):
            for name in files:
                ...
                logger.info(
                    f"Abriendo tabla: '{tbl}' y obteniendo sus columnas... {rows.returns_rows}")
                if(not rows.returns_rows):
                    logger.info(
                        f"No existe tabla con el nombre: '{tbl}', no se procesará el archivo...")
                    continue
                ...
                df = pd.read_csv(_fileName, sep=self.args._sep,
                                 encoding=self.args._encode, dtype=jsonCol)
                _c = str(df.count()[0])
                logger.info(
                    f'Leyendo: {_fileName} -> {_c} registros procesados.')
                ...
                try:
                    ...
                    logger.info(
                        f"Table: {tbl} #Rows: {len(df.index)} {_result}")
        conn.close()
        return _r
