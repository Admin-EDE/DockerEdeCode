...
def fn3EA(conn, return_dict):
    ...
    Returns:
        [Boolean]: [
          ...
          Retorna True y “Aprobado” a través de logger, solo si se puede: 
            - Todas las especialidades están dentro de la lista permitida
    ...
    try:
        especialidadList = check_utils.convertirArray2DToList(
            list(set([m[7] for m in rows if m[7] is not None])))
        if(len(especialidadList) > 0):
            _err = set(
                [e for e in especialidadList if not check_utils.validaEspecialidad(e)])
