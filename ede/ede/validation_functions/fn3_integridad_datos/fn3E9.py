...
def fn3E9(conn, return_dict):
    ...
    Returns:
        [Boolean]: [
          ...
          Retorna True y “Aprobado” a través de logger, solo si se puede: 
            - Todos los sectores registrados cumplen con la lista permitida
    ...
    try:
        sectorList = check_utils.convertirArray2DToList(
            list(set([m[6] for m in rows if m[6] is not None])))
        if(len(sectorList) > 0):
            _err = set(
                [e for e in sectorList if not check_utils.validaSector(e)])
