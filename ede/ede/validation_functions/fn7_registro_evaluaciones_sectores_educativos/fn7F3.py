    ...
    try:
        ...
        for x in _weight:
            if (x is None or x > 100 or x <= 0):
                logger.error(
                    f'El peso de la/s calificacion/es esta mal ingresado')
        ...
        for y in _scoreValue:
            if (y is None):
                logger.error(
                    f'Existen Calificaciones mal ingresadas en el establecimiento')
                logger.error(f'Rechazado')
                _err = False

        if(_err):
            logger.info(
                f'Calificaciones con su ponderacion ingresadas correctamente')
